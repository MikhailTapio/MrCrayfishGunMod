...
## JEI 预览显示修正 — 2026-09-09

- 用户截图反馈重型步枪旋转预览出现部分模型不可见。检查发现预览使用全局 ModelView 矩阵且深度为 Z=0，与界面背景处于同一深度范围。
- 改为 GuiGraphics 的局部 PoseStack，在 Z=150 绘制预览；绘制前提交已有 GUI 批次，模型批次在局部变换恢复前提交，避免背景遮挡及全局矩阵影响其他界面内容。
- build 通过：D:/PRTSNote/tmp/cgm-jei-preview-fix.log（23 秒）；git diff --check 通过。未重启用户正在测试的客户端，修正需重启后由用户确认完整旋转周期的显示效果。
